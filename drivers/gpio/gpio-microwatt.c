...
static int mw_gpio_probe(struct platform_device *pdev)
{
	...
	if (rc > 0) {
		...
		mw_gpio->irq = rc;
		mw_gpio->irq_chip.name = "microwatt-gpio";
		mw_gpio->irq_chip.irq_ack = mw_gpio_irq_ack,
		mw_gpio->irq_chip.irq_mask = mw_gpio_irq_mask,
		mw_gpio->irq_chip.irq_unmask = mw_gpio_irq_unmask,
		mw_gpio->irq_chip.irq_set_type = mw_gpio_irq_set_type,

		girq->chip = &mw_gpio->irq_chip;
